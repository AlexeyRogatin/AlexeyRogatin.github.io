...
function render(wireframe, image, tri) {
    if (wireframe) {
...
function draw(img) {
    ...
    if (dirtyTriangles) {
        ...
        calculateGeometry(img);
    }

    for (triangle of triangles) {
        render(true, img, triangle);
    }
}

function calculateGeometry(image) {
    // clear triangles out
    triangles = [];
...
function drawTriangle(ctx, im, x0, y0, x1, y1, x2, y2,
    ...
    /*
    ctx.transform(m11, m12, m21, m22, dx, dy) sets the ctx transform matrix.
